...
async def async_setup_entry(
    ...
    if not servers_data:
        _LOGGER.debug(
            "No servers configured or successfully initialized for manager entry %s. Skipping switch setup.",
            entry.entry_id,
        )
        return
    ...
    for server_name, server_data in servers_data.items():
        ...
        if not coordinator:
            _LOGGER.warning(
                "Coordinator object missing for server '%s' in entry %s. Skipping switch.",
                server_name,
                entry.entry_id,
            )
            continue
    ...
    if switches_to_add:
        _LOGGER.info(
            "Adding %d switch entities for manager entry %s",
            len(switches_to_add),
            entry.entry_id,
        )
...
class MinecraftServerSwitch(
    ...
    def __init__(
        ...
        self._entry = (
            entry  # Store entry if needed, e.g. for accessing entry.data elsewhere
        )
        ...
        if not self.coordinator.data or (
            ...
            if isinstance(self.coordinator.data, dict):
                ...
                if error_type_name in [
                    ServerNotRunningError.__name__,
                    ServerNotFoundError.__name__,
                ]:
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        _LOGGER.info(
            "Switch Entity '%s' (Unique ID: %s) calling start for server_name: '%s'",
            self.entity_id,
            self.unique_id,
            self._server_name,
        )
        try:
            ...
            raise HomeAssistantError(
                f"Failed to start server {self._server_name}: {err}"
            ) from err
        except Exception as err:
            _LOGGER.exception(
                "Unexpected error turning ON server %s: %s", self._server_name, err
            )
            raise HomeAssistantError(
                f"Unexpected error starting server {self._server_name}: {err}"
            ) from err
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            raise HomeAssistantError(
                f"Failed to stop server {self._server_name}: {err}"
            ) from err
        except Exception as err:
            _LOGGER.exception(
                "Unexpected error turning OFF server %s: %s", self._server_name, err
            )
            raise HomeAssistantError(
                f"Unexpected error stopping server {self._server_name}: {err}"
            ) from err
