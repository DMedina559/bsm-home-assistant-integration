...
class BsmRestoreCard extends LitElement {
    ...
    if (this._selectedSensorEntityId) {
      ...
      if (
        ...
        (stateObj !== oldStateObj ||
          JSON.stringify(stateObj.attributes ?? null) !==
            JSON.stringify(oldStateObj?.attributes ?? null))
      ) {
        _LOGGER.debug(
          `Hass update detected for selected sensor '${this._selectedSensorEntityId}'. Processing.`,
        );
        ...
        if (
          JSON.stringify(oldBackupList) !==
            JSON.stringify(this._availableBackups) ||
        ...
        _LOGGER.warn(
          `Selected sensor '${this._selectedSensorEntityId}' disappeared.`,
        );
  ...
  _handleSensorSelection(entityId) {
    ...
    this._targetDeviceId = null;
    this._error = null;
    this._feedback = this._selectedSensorEntityId
      ? `Processing sensor ${this._selectedSensorEntityId}...`
      : "Select a server's backup list sensor.";
    ...
    if (this.hass && this.hass.states[entityId]) {
      ...
      _LOGGER.warn(
        `Sensor ${entityId} not found in hass states immediately after selection.`,
      );
  ...
  _processSelectedSensor(stateObj) {
    if (!stateObj) {
      // Basic check if stateObj is null/undefined
      _LOGGER.warn(
        `_processSelectedSensor called with invalid stateObj for ${this._selectedSensorEntityId}`,
      );
    ...
    _LOGGER.debug(
      `Processing attributes for ${stateObj.entity_id}:`,
      stateObj.attributes,
    );
    ...
    if (
      ...
      if (
        ...
        _LOGGER.warn(
          `Could not determine device_id for ${entityId}. Restore All will be disabled.`,
        );
      }
    }
    ...
    if (
      genericBackupsAttr &&
      typeof genericBackupsAttr === "object" &&
      !Array.isArray(genericBackupsAttr)
    ) {
      ...
      _LOGGER.debug(
        `Found categorized 'all' backup list in attr 'backups'. Categories: ${categories.join(", ")}. Total files: ${foundBackupList.length}`,
      );
      ...
      else if (nameHint.includes("properties"))
        determinedBackupType = "properties";
      else if (nameHint.includes("allowlist"))
        determinedBackupType = "allowlist";
      else if (nameHint.includes("permissions"))
        determinedBackupType = "permissions";
      _LOGGER.debug(
        `Found generic list in attr 'backups', inferred type: ${determinedBackupType}`,
      );
    } else {
      ...
      for (const attrKey of BACKUP_LIST_ATTRIBUTE_CANDIDATES) {
        ...
        if (Array.isArray(attrValue)) {
          ...
          else if (attrKey.includes("properties"))
            determinedBackupType = "properties";
          else if (attrKey.includes("allowlist"))
            determinedBackupType = "allowlist";
          else if (attrKey.includes("permissions"))
            determinedBackupType = "permissions";
          _LOGGER.debug(
            `Found specific list in attr '${attrKey}', determined type: ${determinedBackupType}`,
          );
    ...
    if (foundBackupList) {
      ...
      if (
        JSON.stringify(sortedBackups) !== JSON.stringify(this._availableBackups)
      ) {
        this._availableBackups = sortedBackups;
        this._selectedBackupFile =
          sortedBackups.length > 0 ? sortedBackups[0] : ""; // Default to latest
        _LOGGER.info(
          `Updated backup list. Type: ${this._backupType}, Attr: '${listAttributeName}', Count: ${this._availableBackups.length}`,
        );
      } else {
        _LOGGER.debug("Backup list unchanged.");
      }
      newFeedback =
        this._availableBackups.length === 0
          ? `No valid backup files found in attribute '${listAttributeName}'.`
          : "";
    } else {
  ...
  _handleBackupSelect(ev) {
    this._selectedBackupFile = ev.target.value;
    this.requestUpdate();
  }
  ...
  async _restoreSelectedBackup() {
    if (!this._targetDeviceId) {
      this._error =
        "Target Device ID not found for the selected sensor. Cannot send restore command.";
      _LOGGER.error(this._error);
      this.requestUpdate();
      return;
    }
    if (!this._selectedBackupFile) {
      this._error = `No backup file selected.`;
      this.requestUpdate();
      return;
    }
    if (!this._backupType || this._backupType === "unknown") {
      this._error =
        "Backup type (world/properties/allowlist/permissions/all) could not be determined for the selected sensor. Cannot restore.";
    ...
    await this._callService(
      "restore_backup",
      {
        ...
        restore_type: serviceRestoreType,
        backup_file: serviceBackupFile,
      },
  ...
  async _restoreLatestAll() {
    if (!this._targetDeviceId) {
      this._error =
        "Target Device ID not found for the selected sensor. Cannot restore latest all.";
      _LOGGER.error(this._error);
      this.requestUpdate();
      return;
    }
    ...
    if (
      !confirm(
        `ARE YOU SURE you want to restore the LATEST ALL (world, properties, allowlist, permissions) backups for ${serverDisplayName}? This may overwrite current data.`,
      )
    ) {
  ...
  async _callService(serviceName, serviceData, operationFeedback) {
    ...
    try {
      _LOGGER.debug(
        `Calling ${DOMAIN}.${serviceName} with data: %o`,
        serviceData,
      );
  ...
  render() {
    ...
    const serverContextName =
      this._serverDisplayName ||
      (this._selectedSensorEntityId ? "Selected Sensor" : "No Sensor Selected");

    let backupTypeDisplay = "Backups";
    if (this._backupType && this._backupType !== "unknown") {
      backupTypeDisplay =
        BACKUP_TYPE_DISPLAY_NAMES[this._backupType] || this._backupType;
      if (this._backupType !== "all") {
        // Only append "Backups" for individual types
      ...
      <ha-card header="${cardTitle}">
        <div class="card-content">
          ...
          <ha-selector
            ...
            @value-changed=${(ev) =>
              this._handleSensorSelection(ev.detail.value)}
            .disabled=${this._isLoading}
            required
          ></ha-selector>

          <div class="status-area">
            ${this._feedback && !this._error
          ...
          ${this._selectedSensorEntityId && !this._error && this._backupType
            ? html`
                <div class="section">
                  ...
                  ${this._availableBackups.length > 0
                    ? html`
                        <ha-select
                          ...
                          required
                        >
                          ${this._availableBackups.map(
                            (file) =>
                              html`<mwc-list-item
                                .value=${file}
                                ?selected=${file === this._selectedBackupFile}
                                >${file}</mwc-list-item
                              >`,
                          )}
                        </ha-select>
                        <ha-button
                          ...
                          title=${!this._targetDeviceId
                            ? "Cannot restore: Device ID missing"
                            : this._backupType === "unknown"
                              ? "Cannot restore: Backup type unknown"
                              : !this._selectedBackupFile
                        ...
                        ${this._backupType === "unknown"
                          ? html`<p class="warning">
                              ...
                              backup list. Restore disabled.
                            </p>`
                          : ""}
                        ${!this._targetDeviceId
                          ? html`<p class="warning">
                              Could not determine Device ID for this server.
                              Restore disabled.
                            </p>`
                          : ""}
                      `
                    : html`<p class="info">
                        ...
                        ${this._backupType !== "unknown"
                          ? backupTypeDisplay
                              ...
                              .replace(" backups", "")
                          : ""}
                        backup files found for this sensor, or the list
          ...
          <div class="section">
            ...
            ${this._selectedSensorEntityId && !this._error
              ? html`
                  <p class="info">
                    This attempts to restore the latest world, properties,
                    allowlist, and permissions backups for the server associated
                  ...
                  ${!this._targetDeviceId
                    ? html`<p class="warning">
                        ...
                        Device ID for this server could not be determined from
                        the selected sensor.
                      </p>`
                    : ""}
                `
              : html`
                  <p class="info">
                    Select a sensor above to enable full restore.
                  </p>
                `}
          </div>
        </div>
...
window.customCards.push({
  ...
  description:
    "Restores world, config, or individual file backups for a selected Bedrock server.",
  preview: true,
});

_LOGGER.info(
  `%c BSM-RESTORE-CARD %c LOADED (incl. editor) %c`,
