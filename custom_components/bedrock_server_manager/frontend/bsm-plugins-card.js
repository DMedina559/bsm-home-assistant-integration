...
class BsmPluginsCard extends LitElement {
  ...
  static getConfigForm() {
    return {
      schema: [
        ...
        {
          name: "reload_button_entity",
          selector: {
            entity: { integration: "bedrock_server_manager", domain: "button" },
          },
          required: true,
        },
      ],
      computeLabel: (schema) => {
        ...
        if (schema.name === "manager_entity")
          return "Plugins Sensor (Required)";
        if (schema.name === "reload_button_entity")
          return "Reload Plugins Button (Required)";
        return undefined;
      },
      computeHelper: (schema) => {
        if (schema.name === "title") return "Overrides the default card title";
        if (schema.name === "manager_entity")
          return "Sensor holding plugin statuses (e.g., sensor.bsm_total_plugins)";
        if (schema.name === "reload_button_entity")
          return "Button entity for reloading plugins (e.g., button.bsm_reload_plugins)";
        return undefined;
      },
    };
  }
  ...
  setConfig(config) {
    ...
    if (!config.reload_button_entity) {
      throw new Error(
        "Configuration error: 'reload_button_entity' is required.",
      );
    }
    // Basic validation for entity ID format
    if (
      typeof config.manager_entity !== "string" ||
      !config.manager_entity.includes(".")
    ) {
      throw new Error(
        "'manager_entity' must be a valid entity ID (e.g., sensor.my_plugins_sensor).",
      );
    }
    if (
      typeof config.reload_button_entity !== "string" ||
      !config.reload_button_entity.includes(".")
    ) {
      throw new Error(
        "'reload_button_entity' must be a valid entity ID (e.g., button.my_reload_button).",
      );
    }
  ...
  _updateDataFromHass(newHass, oldHass) {
    ...
    this._managerDeviceId =
      entityReg?.device_id || stateObj.attributes?.device_id || null;
    if (!this._managerDeviceId) {
      _LOGGER.warn(
        `Could not determine device ID for manager entity ${entityId}. Service calls for toggling plugins might fail.`,
      );
    }

    const newPluginsData = stateObj.attributes?.plugins_data;
    let pluginsChanged =
      JSON.stringify(newPluginsData || {}) !==
      JSON.stringify(this._plugins || {});

    if (pluginsChanged) {
      this._plugins =
        newPluginsData && typeof newPluginsData === "object"
          ? { ...newPluginsData }
          : {};
      _LOGGER.debug(
        "Plugin data updated from sensor:",
        entityId,
        "Found plugins:",
        Object.keys(this._plugins).length,
      );
    }
  ...
  _togglePlugin(pluginName, currentStatus) {
    ...
    this.hass
      .callService(
        ...
        { device_id: this._managerDeviceId },
      ) // Corrected Target (4th argument is the ServiceTarget object)
      .then(() => {
        this._feedback = `Plugin '${pluginName}' toggle request sent. State will update via sensor.`;
        _LOGGER.info(
          `Toggled plugin ${pluginName} to ${!currentStatus} for device ${this._managerDeviceId}`,
        );
      })
      .catch((err) => {
  ...
  _reloadPlugins() {
    if (!this.hass || !this.config.reload_button_entity) {
      this._error =
        "Cannot reload plugins: Reload button entity not configured.";
      _LOGGER.error(this._error);
      this.requestUpdate();
      return;
    }
    ...
    this.hass
      ...
      .then(() => {
        this._feedback =
          "Plugin reload request sent. Status will update via sensor.";
  ...
  render() {
    if (
      !this.config ||
      !this.config.manager_entity ||
      !this.config.reload_button_entity
    ) {
      return html`
        <ha-card header="Bedrock Server Plugins">
          <div class="card-content">
            <p>
              Error: Card not fully configured. Please set 'manager_entity' and
              'reload_button_entity'.
            </p>
          </div>
        </ha-card>
      `;
    ...
    if (!this.hass) {
      return html`<ha-card
        header="${this.config.title || "Bedrock Server Plugins"}"
    ...
    if (!managerState) {
      return html`
        <ha-card header="${this.config.title || "Bedrock Server Plugins"}">
          <div class="card-content">
            <p>
              Entity not found: ${this.config.manager_entity}. Please check your
              configuration.
            </p>
            ${this._feedback
          ...
          ${pluginEntries.length === 0 && !this._isLoading && !this._error
            ...
            : pluginEntries.map(([name, details]) => {
                if (typeof details !== "object" || details === null) {
                  ...
                  return html`<div class="plugin-entry error">
                    Plugin '${name}' has invalid data structure.
                  </div>`;
                }
                return html` <div class="plugin-entry">
                  <div class="plugin-info">
                    <div class="plugin-name">${name}</div>
                    <div class="plugin-version">
                      v${details.version || "N/A"}
                    </div>
...
_LOGGER.info(
  `%cBSM-PLUGINS-CARD ${CARD_VERSION} IS INSTALLED%c (incl. editor)`,
...
const existingCardIndex = window.customCards.findIndex(
  (card) => card.type === "bsm-plugins-card",
);
