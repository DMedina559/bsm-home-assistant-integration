...
class BsmContentInstallerCard extends LitElement {
  ...
  static getConfigForm() {
    return {
      schema: [
        ...
        {
          name: "content_source_sensor_entity",
          selector: {
      ...
      computeLabel: (schema) => {
        ...
        if (schema.name === "content_source_sensor_entity")
          return "Content Source Sensor (Optional)";
        return undefined;
      },
      computeHelper: (schema) => {
        if (schema.name === "title") return "Overrides the default card title";
        if (schema.name === "content_source_sensor_entity")
          return "Pre-select the sensor providing the content list";
        return undefined;
      },
    };
  }
  ...
  set hass(hass) {
    ...
    if (hass && this._selectedContentSourceSensorId) {
      ...
      const oldSourceStateObj =
        oldHass?.states[this._selectedContentSourceSensorId];

      // Check if state object or its attributes actually changed
      if (
        sourceStateObj &&
        (sourceStateObj !== oldSourceStateObj ||
          JSON.stringify(sourceStateObj.attributes ?? null) !==
            JSON.stringify(oldSourceStateObj?.attributes ?? null))
      ) {
        _LOGGER.debug(
          `Hass update for content source sensor ${this._selectedContentSourceSensorId}`,
        );
        this._processSelectedContentSourceSensor(sourceStateObj);
      } else if (!sourceStateObj && oldSourceStateObj) {
        _LOGGER.warn(
          `Content source sensor ${this._selectedContentSourceSensorId} disappeared.`,
        );
      ...
      !this._selectedContentSourceSensorId &&
      this.config?.content_source_sensor_entity
    ) {
      ...
      if (configStateObj) {
        _LOGGER.debug(
          `Initial processing of configured sensor ${configSensorId} after hass became available.`,
        );
  ...
  setConfig(config) {
    ...
    if (initialSensorId !== this._selectedContentSourceSensorId) {
      _LOGGER.debug(
        `Setting internal _selectedContentSourceSensorId from config: ${initialSensorId}`,
      );
      this._handleContentSourceSensorChange(initialSensorId); // Use the handler to manage state reset
    } else if (
      initialSensorId &&
      this.hass &&
      this.hass.states[initialSensorId]
    ) {
      ...
      this._processSelectedContentSourceSensor(
        this.hass.states[initialSensorId],
      );
    }
  ...
  _handleContentSourceSensorChange(entityId) {
    _LOGGER.debug(
      `_handleContentSourceSensorChange called with entityId: ${entityId}`,
    );
    ...
    if (this.hass && this.hass.states[this._selectedContentSourceSensorId]) {
      this._processSelectedContentSourceSensor(
        this.hass.states[this._selectedContentSourceSensorId],
      );
    } else {
      _LOGGER.warn(
        `Content source sensor ${this._selectedContentSourceSensorId} not found in hass states immediately after selection.`,
      );
  ...
  _processSelectedContentSourceSensor(stateObj) {
    if (!stateObj?.attributes) {
      const sensorId =
        stateObj?.entity_id || this._selectedContentSourceSensorId;
    ...
    _LOGGER.debug(
      `Processing attributes for content source ${stateObj.entity_id}:`,
      stateObj.attributes,
    );
    ...
    if (
      stateObj.attributes.available_worlds_list &&
      Array.isArray(stateObj.attributes.available_worlds_list)
    ) {
      determinedInstallType = "world";
      listAttributeName = "available_worlds_list";
      foundList = stateObj.attributes.available_worlds_list;
    } else if (
      stateObj.attributes.available_addons_list &&
      Array.isArray(stateObj.attributes.available_addons_list)
    ) {
      ...
      for (const attrKey of CONTENT_LIST_ATTRIBUTE_CANDIDATES) {
        ...
        if (
          Array.isArray(stateObj.attributes[attrKey]) &&
          stateObj.attributes[attrKey].length > 0
        ) {
          ...
          const nameHint = (
            ...
            stateObj.entity_id.split(".").pop() ||
            ""
          ).toLowerCase();
          if (nameHint.includes("world")) determinedInstallType = "world";
          else if (nameHint.includes("addon") || nameHint.includes("pack"))
            determinedInstallType = "addon";
          else
            _LOGGER.warn(
              `Could not infer type ('world'/'addon') from name hint: '${nameHint}'`,
            );
          _LOGGER.debug(
            `Found generic list in attribute '${attrKey}', inferred type: ${determinedInstallType}`,
          );
          break; // Stop searching once a list is found
        } else if (stateObj.attributes[attrKey] !== undefined) {
          _LOGGER.debug(
            `Attribute '${attrKey}' found but is not a non-empty array:`,
            stateObj.attributes[attrKey],
          );
        }
      }
    }
    ...
    if (foundList) {
      ...
      if (
        ...
        JSON.stringify(this._availableFilesForInstall)
      ) {
        ...
        _LOGGER.info(
          `Content source: ${stateObj.entity_id}. Determined type: ${this._installType}, list from attr: '${listAttributeName}', Valid files found: ${newList.length}`,
        );
      } else {
      ...
      this._feedback =
        this._availableFilesForInstall.length === 0
      ...
      _LOGGER.warn(
        `Could not find a recognized and populated content list attribute on ${stateObj.entity_id}. Checked: ${CONTENT_LIST_ATTRIBUTE_CANDIDATES.join(", ")}`,
      );
  ...
  _handleTargetServerChange(ev) {
    ...
    _LOGGER.debug(
      "Target Server Device ID selected:",
      this._selectedTargetServerDeviceId,
    );
    this.requestUpdate();
  }
  ...
  async _installContent() {
    ...
    if (!this._selectedContentSourceSensorId) {
      this._error = "Error: Content source sensor not selected.";
      this.requestUpdate();
      return;
    }
    if (!this._selectedTargetServerDeviceId) {
      this._error = "Error: Target server device not selected.";
      this.requestUpdate();
      return;
    }
    if (!this._installType || this._installType === "unknown") {
      this._error =
        "Error: Content type (world/addon) could not be determined from the source sensor. Check sensor attributes.";
      this.requestUpdate();
      return;
    }
    if (!this._selectedFileToInstall) {
      this._error = "Error: No file selected to install.";
      this.requestUpdate();
      return;
    }

    const serviceName =
      this._installType === "world" ? "install_world" : "install_addon";
    const serviceData = {
  ...
  render() {
    if (!this.hass) {
      return html`<ha-card>Waiting for Home Assistant...</ha-card>`;
    }
      ...
      <ha-card header="${cardTitle}">
        <div class="card-content">
          ...
          <ha-selector
            ...
            @value-changed=${(ev) =>
              this._handleContentSourceSensorChange(ev.detail.value)}
            helper="Sensor listing available .mcworld, .mcpack, etc. (e.g., from FileList sensor)"
            ?required=${!this._selectedContentSourceSensorId}
          ></ha-selector>
                ...
                ${this._availableFilesForInstall.length > 0
                  ? html`
                      ...
                      >
                        ...
                        ${this._availableFilesForInstall.map(
                          (file) =>
                            html`<mwc-list-item .value=${file}
                              >${file}</mwc-list-item
                            >`,
                        )}
                      </ha-select>
                      ${this._installType === "unknown"
                        ? html`<p class="warning">
                            ...
                            (World/Addon) from the source sensor's attributes or
                            name. Installation may fail or use the wrong service
                            call. Ensure sensor has 'available_worlds_list',
                            'available_addons_list', or a name hint.
                          </p>`
                        : ""}
                    `
                  : html`<p class="info">
                      No compatible files found in the selected content source
                      sensor, or the list attribute (e.g.,
                      'available_worlds_list') was not found or empty.
                    </p>`}
              `
            : ""}
          ...
          <div class="status-area">
            ${this._isLoading
              ...
              : ""}
            ${!this._isLoading && this._feedback
  ...
  static get styles() {
    return css`
      ...
      ha-select {
        ...
        --mdc-menu-min-width: calc(100% - 32px); /* Adjust as needed */
      }
      h4 {
...
window.customCards.push({
  ...
  description:
    "Installs worlds or addons from a selected list sensor to a Bedrock server.",
  preview: true, // Preview should be okay as basic structure is shown
});

_LOGGER.info(
  `%c BSM-CONTENT-INSTALLER-CARD %c LOADED (incl. editor) %c`,
  "color: teal; font-weight: bold; background: black",
