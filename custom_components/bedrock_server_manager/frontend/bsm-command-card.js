// custom_components/bedrock_server_manager/frontend/bsm-command-card.js
import {
  LitElement,
  html,
  css,
...
class BsmCommandCard extends LitElement {
  ...
  static get styles() {
    return css`
      ...
      .card-content {
        flex-grow: 1;
        padding: 16px;
        padding-bottom: 8px; /* Added top padding */
      }
      .card-actions {
      ...
      ha-selector[label^="Command"] {
        margin-bottom: 8px;
      } /* Reduce margin below command */
      .feedback-area {
  ...
  render() {
    if (!this.hass) {
      return html`<ha-card header="Send Command"
    ...
    const isButtonDisabled =
      !this._commandText?.trim() || !this._selectedDeviceId || this._isLoading;
      ...
      <ha-card header="${cardTitle}">
        <div class="card-content">
          ...
          <ha-textfield
            ...
            .value=${this._commandText}
            @input=${this
            ._handleCommandInput} // Need this handler if using ha-textfield
            ?disabled=${this._isLoading}
            required
...
_LOGGER.info(
  `%c BSM-COMMAND-CARD %c LOADED (incl. editor) %c`,
