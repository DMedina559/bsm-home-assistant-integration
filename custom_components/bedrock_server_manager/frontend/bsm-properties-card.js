...
class BsmPropertiesCard extends LitElement {
  ...
  set hass(hass) {
    ...
    if (this._selectedStatusEntityId) {
      ...
      if (stateObj) {
        ...
        if (
          JSON.stringify(currentPropsAttr ?? null) !==
          JSON.stringify(oldPropsAttr ?? null)
        ) {
          _LOGGER.debug(
            "Hass update detected server_properties change for:",
          ...
          if (
            ...
            JSON.stringify(this._currentProperties ?? {})
          ) {
            updateNeeded = true;
          }
        }
        ...
        if (
          ...
          _LOGGER.warn(
            "Selected sensor %s disappeared or not found after being selected.",
            this._selectedStatusEntityId,
          );
  ...
  constructor() {
    super();
    this._initialFeedbackMessage =
      "Select the sensor holding server properties.";
  ...
  _handleEntitySelection(entityId) {
    ...
    if (stateObj) {
      _LOGGER.debug(
        "Entity %s found, loading properties.",
        this._selectedStatusEntityId,
      );
      this._loadProperties(stateObj);
  ...
  _loadProperties(stateObj) {
    ...
    _LOGGER.debug(
      `_loadProperties: Examining 'server_properties' attribute for ${stateObj.entity_id}:`,
    ...
    if (
      ...
      if (newPropsString !== currentPropsString) {
        _LOGGER.debug(
          "Loading new/updated server_properties from state for %s",
          stateObj.entity_id,
        );
        ...
        _LOGGER.debug(
          "'server_properties' attribute unchanged or edits in progress, not overwriting _editValues for %s",
          stateObj.entity_id,
        );
      }
    } else {
      _LOGGER.warn(
        "'server_properties' attribute missing, not an object, or empty on %s",
        stateObj.entity_id,
      );
  ...
  _renderPropertySelector(propKey) {
    ...
    let label = propKey
      .replace(/-/g, " ")
      .replace(/\b\w/g, (l) => l.toUpperCase()); // This label is now for ha-selector
    switch (propKey) {
      case "server-name":
      case "level-name":
      case "level-seed":
      ...
      case "enable-lan-visibility":
      case "allow-list":
        selectorConfig = { boolean: {} };
        break;
      case "max-players":
      case "server-port":
      case "server-portv6":
      ...
      case "tick-distance":
        let minVal = 0,
          ...
          stepVal = 1,
          mode = "box";
        if (propKey === "max-players") {
          minVal = 1;
          maxVal = 200;
        } else if (propKey === "view-distance") {
          minVal = 3;
          maxVal = 32;
          mode = "slider";
        } else if (propKey === "tick-distance") {
          minVal = 4;
          maxVal = 12;
        } else if (propKey === "server-port" || propKey === "server-portv6") {
          minVal = 1;
          maxVal = 65535;
        }
        selectorConfig = {
          number: { min: minVal, max: maxVal, step: stepVal, mode: mode },
        };
        break;
    ...
    return html`
      <div
        ...
        title=${`Current saved value: ${currentValue ?? "Not set"}`}
      >
        ...
        <ha-selector
          ...
          .label=${label}
          @value-changed=${(ev) =>
            this._handleValueChange(propKey, ev.detail.value)}
          ...
          ${canDisplayProperties
            ? html`
                <div class="property-grid">
                  ${EDITABLE_PROPERTIES.filter((propKey) =>
                    this._currentProperties.hasOwnProperty(propKey),
                  ).map((propKey) => this._renderPropertySelector(propKey))}
                </div>
              `
            : ""}
          ${this._isLoading && this._selectedStatusEntityId
  ...
  async _saveProperties() {
    ...
    if (!targetDeviceId) {
      this._error =
        "Error: Could not determine target device ID from the selected sensor.";
      this._feedback = "";
      this.requestUpdate();
      return;
    }
    ...
    if (!actuallyChanged) {
      this._error = "No changes detected to save.";
      this._feedback = "";
      this.requestUpdate();
      return;
    }
    const success = await this._callService(
      "update_properties",
      { device_id: targetDeviceId, properties: changedProperties },
  ...
  getCardSize() {
    ...
    if (
      ...
      numPropsRendered = EDITABLE_PROPERTIES.filter((propKey) =>
        this._currentProperties.hasOwnProperty(propKey),
      ).length;
    }
...
_LOGGER.info(
  `%c BSM-PROPERTIES-CARD %c LOADED (incl. editor) %c`,
  "color: blue; font-weight: bold; background: white",
