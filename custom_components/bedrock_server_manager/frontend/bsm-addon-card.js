...
class BsmAddonCard extends LitElement {
    static getConfigForm() {
        return {
            schema: [
                ...
                {
                    name: "target_server_sensor_entity",
                    selector: {
            ...
            computeLabel: (schema) => {
                ...
                if (schema.name === "target_server_sensor_entity")
                    return "Target Server Sensor";
                return undefined;
            },
        };
    }
    ...
    setConfig(config) {
        ...
        this.config = config;
        if (
            this.config.target_server_sensor_entity &&
            this.config.target_server_sensor_entity !==
            this._selectedTargetServerSensorId
        ) {
            this._handleTargetServerSensorChange(
                this.config.target_server_sensor_entity,
            );
        }
    }
    ...
    set hass(hass) {
        ...
        if (hass && this._selectedTargetServerSensorId) {
            ...
            const oldSourceStateObj =
                oldHass?.states[this._selectedTargetServerSensorId];
    ...
    _handleTargetServerSensorChange(entityId) {
        ...
        if (this.hass && this.hass.states[this._selectedTargetServerSensorId]) {
            this._processSelectedTargetServerSensor(
                this.hass.states[this._selectedTargetServerSensorId],
            );
        } else {
    ...
    _processSelectedTargetServerSensor(stateObj) {
        ...
        if (stateObj.attributes.server_addons_list) {
            ...
            let bp = (addonsData.behavior_packs || []).map((p) => ({
                ...p,
                id: p.uuid,
            }));
            let rp = (addonsData.resource_packs || []).map((p) => ({
                ...p,
                id: p.uuid,
            }));
    ...
    async handleAddonAction(pack, packType, action) {
        ...
        await this._performAction(
            serviceName,
            { pack_uuid: pack.uuid, pack_type: packType },
            `${action} successful.`,
        );
    }
    ...
    async handleSaveAddonOrder() {
        const listKey =
            this._activeTab === "behavior" ? "behavior_packs" : "resource_packs";
      ...
      <div class="addon-item">
        ...
        ${isActive && hasSubpacks
              ...
              <div class="subpack-selector">
                ...
                <select
                  @change=${(e) =>
                        this.handleSubpackChange(item, packType, e.target.value)}
                  ?disabled=${this._isLoading}
                >
                  ${subpacks.map(
                            (sp) =>
                                html`<option value="${sp.folder_name}" ?selected=${sp.folder_name === activeSubpack}>
                        ${sp.name}
                      </option>`,
                        )}
                </select>
    ...
    render() {
        ...
        const currentList =
            this._activeTab === "behavior"
                ? this._addons.behavior_packs
                : this._addons.resource_packs;
      ...
      <ha-card header="${cardTitle}">
        <div class="card-content">
          <ha-selector
            ...
            @value-changed=${(ev) =>
                this._handleTargetServerSensorChange(ev.detail.value)}
                  ...
                  ${currentList.length > 0
                      ...
                      `
                        ...
                        <div class="empty-state">
                          No ${this._activeTab} packs installed.
                        </div>
                      `}
                </div>
...
window.customCards.push({
    ...
    description:
        "Manage installed behavior and resource packs on a Bedrock server.",
    preview: true,
});
