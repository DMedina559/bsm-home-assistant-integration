...
class BsmAllowlistCard extends LitElement {
  ...
  static get styles() {
    return css`
      :host {
        display: block;
      } /* Ensure the card itself behaves as a block */
      ha-card {
        display: flex;
      ...
      .allowlist-section ul {
        list-style: none;
        padding: 0;
        margin: 0;
      }
      .allowlist-section li {
        display: flex;
        justify-content: space-between;
        align-items: center;
        padding: 4px 0;
      }
      .allowlist-section li:not(:last-child) {
        border-bottom: 1px solid var(--divider-color);
      }
      .allowlist-section span {
        flex-grow: 1;
        margin-right: 8px;
  ...
  set hass(hass) {
    ...
    if (!hass) {
      _LOGGER.warn(
        "Hass object became undefined. Card may not function correctly.",
      );
    ...
    if (this._selectedStatusEntityId) {
      ...
      if (stateObj) {
        ...
        if (
          ...
          JSON.stringify(currentAllowlistAttr ?? null) !==
            JSON.stringify(oldAllowlistAttr ?? null)
        ) {
          _LOGGER.debug(
            "Hass update for selected entity:",
            this._selectedStatusEntityId,
          );
        ...
        if (
          oldStateObj ||
          (!this._error && this._feedback !== this._initialFeedbackMessage)
        ) {
          _LOGGER.warn(
            "Allowlist entity %s disappeared or not found after being selected.",
            this._selectedStatusEntityId,
          );
      ...
      const isDefaultState =
        (this._currentAllowlist?.length ?? 0) === 0 &&
        !this._error &&
        (this._feedback === this._initialFeedbackMessage || !this._feedback);
      if (!isDefaultState) {
        _LOGGER.debug(
          "No entity selected. Resetting to default feedback state.",
        );
  ...
  _resetSelectionRelatedState(feedbackMessage = "", error = null) {
    _LOGGER.debug(
      "Resetting selection-related state. Feedback:",
      feedbackMessage,
      "Error:",
      error,
    );
  ...
  _handleEntitySelection(entityId) {
    ...
    if (!this.hass) {
      ...
      this._error =
        "Home Assistant data not yet available. Please wait or refresh.";
      this._feedback = "";
      this.requestUpdate();
      return;
    }
    ...
    if (stateObj) {
      _LOGGER.debug(
        "Entity %s found, loading allowlist.",
        this._selectedStatusEntityId,
      );
      this._loadAllowlist(stateObj);
      if (!this._error) {
      ...
      _LOGGER.warn(
        `Selected entity ${this._selectedStatusEntityId} not found in current HASS states.`,
      );
      this._resetSelectionRelatedState(
        "",
  ...
  _loadAllowlist(stateObj) {
    ...
    if (!stateObj?.attributes) {
      _LOGGER.warn(
        "State object missing attributes for %s",
        stateObj?.entity_id ?? "unknown entity",
      );
    ...
    if (allowlist && Array.isArray(allowlist)) {
      if (
        ...
        JSON.stringify(this._currentAllowlist ?? [])
      ) {
        _LOGGER.debug(
          "Loading new allowlist for %s: %o",
          stateObj.entity_id,
          allowlist,
        );
      ...
      _LOGGER.warn(
        "'allowed_players' attribute missing or not an array on %s",
        stateObj.entity_id,
      );
  ...
  _handleIgnoreLimitToggle(ev) {
    ...
    _LOGGER.debug(
      "Ignores Player Limit toggle changed:",
      this._addIgnoresLimit,
    );
  }
  ...
  _getTargetDeviceId() {
    if (!this.hass || !this._selectedStatusEntityId) {
      _LOGGER.warn(
        "_getTargetDeviceId: HASS object or selected entity ID is not available.",
      );
      return null;
    }
    ...
    if (
      typeof deviceIdFromAttr === "string" &&
      deviceIdFromAttr.trim() !== ""
    ) {
      deviceId = deviceIdFromAttr.trim();
      _LOGGER.debug(
        "_getTargetDeviceId: Found device_id in state attributes for %s: %s",
        entityId,
        deviceId,
      );
      return deviceId;
      ...
      _LOGGER.warn(
        "_getTargetDeviceId: 'device_id' found in state attributes for %s, but it's not a non-empty string: %o",
        entityId,
        deviceIdFromAttr,
      );
    }
    ...
    if (
      typeof deviceIdFromRegistry === "string" &&
      deviceIdFromRegistry.trim() !== ""
    ) {
      deviceId = deviceIdFromRegistry.trim();
      _LOGGER.debug(
        "_getTargetDeviceId: Found device_id in hass.entities for %s: %s",
        entityId,
        deviceId,
      );
      return deviceId;
      ...
      _LOGGER.warn(
        "_getTargetDeviceId: 'device_id' found in hass.entities for %s, but it's not a non-empty string: %o",
        entityId,
  ...
  async _addPlayer() {
    ...
    if (!targetDeviceId) {
      this._error =
        "Cannot add player: Target device ID for the selected server is missing or invalid. Please check the sensor entity's configuration and attributes in Home Assistant.";
      this._feedback = "";
      this.requestUpdate();
      return;
    }
  ...
  async _removePlayer(playerName) {
    ...
    if (!targetDeviceId) {
      this._error =
        "Cannot remove player: Target device ID for the selected server is missing or invalid. Please check the sensor entity's configuration and attributes in Home Assistant.";
      this._feedback = "";
      this.requestUpdate();
      return;
    }
  ...
  render() {
    ...
    const isPlayerAlreadyAdded =
      this._currentAllowlist?.includes(this._newPlayerName?.trim() ?? "") ??
      false;
    ...
    return html`
      <ha-card header="${cardTitle}">
        <div class="card-content">
          <ha-selector
            .label=${this._selectedStatusEntityId
              ? "Selected Server Allowlist Sensor"
              : "Select Server Allowlist Sensor"}
            .hass=${this.hass}
            .selector=${entitySelectorConfig}
            .value=${this._selectedStatusEntityId}
            @value-changed=${(ev) =>
              this._handleEntitySelection(ev.detail.value)}
          ></ha-selector>
          ...
          ${this._isLoading
            ...
            : ""}
          ${this._feedback && !this._isLoading
            ? html`<div class="feedback">${this._feedback}</div>`
            : ""}
          ${this._error && !this._isLoading
            ? html`<div class="error">${this._error}</div>`
            : ""}
          ${this._selectedStatusEntityId && !this._error && !this._isLoading
            ? html`
                <div class="allowlist-section">
                  <h4>
                    Current Allowlist (${this._currentAllowlist?.length ?? 0}):
                  </h4>
                  ${(this._currentAllowlist?.length ?? 0) === 0
                    ? html`<p>Allowlist is empty.</p>`
                    : ""}
                  <ul>
                    ${(this._currentAllowlist ?? []).map(
                      (playerName) => html`
                        <li>
                  ...
                  <div class="add-player-options">
                    ...
                    <label
                      @click=${(e) => {
                        ...
                        if (!this._isLoading)
                          this.shadowRoot
                            .querySelector(".add-player-options ha-switch")
                            ?.click();
                      }}
                    >
          ...
          ${this._selectedStatusEntityId && !this._error && this._isLoading
            ? html`
                <div>
                  <!-- Placeholder while loading to prevent layout shift if content would disappear -->
                </div>
              `
            : ""}
        </div>
...
_LOGGER.info(
  "%c BSM-ALLOWLIST-CARD %c LOADED (incl. editor) %c",
