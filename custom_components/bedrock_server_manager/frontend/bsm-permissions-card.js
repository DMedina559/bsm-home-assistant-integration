// custom_components/bedrock_server_manager/frontend/bsm-permissions-card.js
import {
  LitElement,
  html,
  css,
...
class BsmPermissionsCard extends LitElement {
  ...
  static getConfigForm() {
    return {
      schema: [
        ...
        {
          name: "global_players_entity",
          selector: {
            entity: { integration: "bedrock_server_manager", domain: "sensor" },
          },
          required: true,
        },
      ],
      computeLabel: (schema) => {
        ...
        if (schema.name === "global_players_entity")
          return "Global Players Sensor (Required)";
        return undefined;
      },
      computeHelper: (schema) => {
        if (schema.name === "title") return "Overrides the default card title";
        if (schema.name === "global_players_entity")
          return "Sensor providing the list of global players";
        return undefined;
      },
    };
  }
  ...
  static getStubConfig(hass) {
    ...
    if (hass) {
      ...
      if (potentialSensors.length > 0) {
        ...
        _LOGGER.debug(
          `StubConfig: Found potential default global players sensor: ${defaultGlobalPlayerSensor}`,
        );
      } else {
        _LOGGER.debug(
          "StubConfig: Could not find a default global players sensor heuristically.",
        );
      }
    }
    return {
  ...
  set hass(hass) {
    ...
    if (globalPlayersEntityId) {
      ...
      if (globalStateObj && globalStateObj !== oldGlobalStateObj) {
        ...
        if (
          JSON.stringify(oldGlobalPlayers) !==
          JSON.stringify(this._globalPlayers)
        ) {
          updateNeeded = true;
        }
      } else if (!globalStateObj && oldGlobalStateObj) {
        _LOGGER.warn(
          `Global players entity ${globalPlayersEntityId} became unavailable.`,
        );
        this._globalPlayers = [];
        updateNeeded = true;
      }
      ...
      _LOGGER.warn(
        "Global players entity not configured, clearing existing global players list.",
      );
      this._globalPlayers = [];
      updateNeeded = true;
    }
    ...
    if (this._selectedServerPermsEntityId) {
      ...
      const oldServerStateObj =
        oldHass?.states[this._selectedServerPermsEntityId];
      if (serverStateObj) {
        ...
        if (
          serverStateObj !== oldServerStateObj ||
          JSON.stringify(currentAttr ?? null) !==
            JSON.stringify(oldAttr ?? null)
        ) {
          const oldServerPerms = this._currentServerPermissions;
          this._loadServerPermissions(serverStateObj);
          if (
            JSON.stringify(oldServerPerms) !==
            JSON.stringify(this._currentServerPermissions)
          ) {
            updateNeeded = true;
          }
        }
        ...
        _LOGGER.warn(
          `Selected server permissions entity ${this._selectedServerPermsEntityId} became unavailable.`,
        );
  ...
  setConfig(config) {
    ...
    if (
      !config.global_players_entity ||
      typeof config.global_players_entity !== "string" ||
      !config.global_players_entity.includes(".")
    ) {
      _LOGGER.error(
        "Invalid configuration: 'global_players_entity' is missing or invalid.",
        config,
      );
      throw new Error(
        "Invalid configuration: 'global_players_entity' is required. Please configure this in the UI editor.",
      );
    }
    ...
    if (
      oldConfig?.global_players_entity !== this.config.global_players_entity
    ) {
      _LOGGER.debug(
        `Global players entity changed from ${oldConfig?.global_players_entity} to ${this.config.global_players_entity}. Reloading.`,
      );
      this._globalPlayers = [];
      if (this.hass && this.hass.states[this.config.global_players_entity]) {
        this._loadGlobalPlayers(
          this.hass.states[this.config.global_players_entity],
        );
      } else if (this.hass) {
        _LOGGER.warn(
          `Configured global players entity ${this.config.global_players_entity} not found in current hass state.`,
        );
      } else {
        _LOGGER.debug(
          "Hass not available yet, global players will load when hass updates.",
        );
      }
    }
  ...
  _loadGlobalPlayers(stateObj) {
    if (!stateObj?.attributes) {
      _LOGGER.warn(
        `Global players entity ${stateObj?.entity_id || this.config?.global_players_entity} is missing attributes.`,
      );
      this._globalPlayers = [];
    ...
    if (players && Array.isArray(players)) {
      ...
      if (newPlayers.length !== players.length) {
        _LOGGER.warn(
          "Some items in 'global_players_list' attribute were filtered out due to missing 'xuid' or 'name'.",
          players,
        );
      }
    } else {
      _LOGGER.warn(
        `Global players attribute 'global_players_list' missing or not an array on ${stateObj.entity_id}.`,
      );
    }
  ...
  _handleServerEntitySelection(entityId) {
    _LOGGER.debug(
      `_handleServerEntitySelection called with entityId: ${entityId}`,
    );
    ...
    if (this.hass && this.hass.states[this._selectedServerPermsEntityId]) {
      this._loadServerPermissions(
        this.hass.states[this._selectedServerPermsEntityId],
      );
    } else {
      _LOGGER.warn(
        `Selected server entity ${this._selectedServerPermsEntityId} not found in hass states immediately.`,
      );
  ...
  _loadServerPermissions(stateObj) {
    ...
    if (permissions && Array.isArray(permissions)) {
      newPermissions = permissions.filter(
        (p) =>
          ...
          typeof p === "object" &&
          p.xuid &&
          VALID_PERMISSIONS.includes(p.permission_level),
      );
      if (newPermissions.length !== permissions.length) {
        _LOGGER.warn(
          "Some items in 'server_permissions' attribute were filtered out due to missing/invalid fields.",
          permissions,
        );
      }
    } else {
      _LOGGER.warn(
        `'server_permissions' attribute missing or not an array on ${stateObj.entity_id}.`,
      );
    ...
    if (
      ...
      _LOGGER.debug(
        `Loading new server permissions data for ${stateObj.entity_id}:`,
      ...
      if (this._feedback === "Loading server permissions...")
        this._feedback = "";
      this.requestUpdate();
    }
  }
  ...
  _stageNewPlayerPermission() {
    ...
    if (
      this._currentServerPermissions.some((p) => p.xuid === xuidToAdd) ||
      this._editData.hasOwnProperty(xuidToAdd)
    ) {
    ...
    this._editData = {
      ...this._editData,
      [xuidToAdd]: this._newPlayerPermissionLevel,
    };
  ...
  async _savePermissions() {
    if (!this._selectedServerPermsEntityId) {
      this._error = "Error: No server selected.";
      this.requestUpdate();
      return;
    }
    if (Object.keys(this._editData).length === 0) {
      this._error = "Error: No changes are staged for saving.";
      this.requestUpdate();
      return;
    }
    this._isLoading = true;
    this._error = null;
    this._feedback = "Saving permissions changes...";
    this.requestUpdate();
    let targetDeviceId = null;
    try {
      const entityReg = await this.hass.callWS({
        type: "config/entity_registry/get",
        entity_id: this._selectedServerPermsEntityId,
      });
      targetDeviceId = entityReg?.device_id;
      _LOGGER.debug("Device ID from Entity Registry:", targetDeviceId);
    } catch (e) {
      _LOGGER.warn(
        `Could not get entity registry info for ${this._selectedServerPermsEntityId}:`,
        e,
      );
      const entityState = this.hass?.states[this._selectedServerPermsEntityId];
      targetDeviceId = entityState?.attributes?.device_id;
      _LOGGER.debug(
        "Device ID fallback from state attributes:",
        targetDeviceId,
      );
    }
    if (!targetDeviceId) {
    ...
    try {
      ...
      _LOGGER.debug(
        `Calling ${DOMAIN}.set_permissions for device_id: ${targetDeviceId} with data:`,
        serviceData,
      );
      await this.hass.callService(DOMAIN, "set_permissions", serviceData);
      this._feedback =
        "Permissions update successfully requested. The list will refresh automatically when the server reports the changes.";
      this._editData = {};
    } catch (err) {
  ...
  render() {
    ...
    if (!this.config?.global_players_entity) {
      return html`<ha-card>
        <div class="card-content error">
          Configuration Error: 'Global Players Entity' is not set. Please
          configure this card.
        </div>
      </ha-card>`;
    }
    ...
    const canAddPlayer =
      !isLoading &&
      (!this._newPlayerSelectedXUID && !this._newPlayerManualXUID.trim()
        ? false
        : true);
    ...
    const availableGlobalPlayerOptions = this._globalPlayers
      ...
      .map((p) => ({
        value: p.xuid,
        label: `${p.name || "Unnamed Player"} (${p.xuid})`,
      }));
    const combinedPermissionsList = [
      ...
      ...Object.keys(this._editData)
        .filter(
          (xuid) =>
            !this._currentServerPermissions.some((p) => p.xuid === xuid),
        )
        .map((xuid) => {
          const globalPlayer = this._globalPlayers.find(
            (gp) => gp.xuid === xuid,
          );
          const manualName =
            xuid === this._newPlayerManualXUID ? this._newPlayerManualName : "";
          return {
            xuid,
      ...
      <ha-card header="${cardTitle}">
        <div class="card-content">
          ...
          <ha-selector
            ...
            @value-changed=${(ev) =>
              this._handleServerEntitySelection(ev.detail.value)}
            .disabled=${isLoading}
            required
          ></ha-selector>
          <div class="status-area">
            ${this._feedback
              ? html`<div class="feedback">${this._feedback}</div>`
              : ""}
          ...
          ${this._selectedServerPermsEntityId
            ? html`
                <div class="section">
                  <h4>Current Server Permissions:</h4>
                  ${combinedPermissionsList.length === 0
                    ? html`<p class="info">
                        No permissions currently set or staged for this server.
                      </p>`
                    : ""}
                  ${combinedPermissionsList.map((p) => {
                    const currentLevel =
                      this._editData[p.xuid] || p.permission_level;
                    const originalPermission =
                      this._currentServerPermissions.find(
                        (cp) => cp.xuid === p.xuid,
                      );
                    const originalLevel = originalPermission?.permission_level;
                    const isModified =
                      p.isNewStaged ||
                      (originalLevel && currentLevel !== originalLevel);
                    return html`
                      <div
                        class="permission-entry ${isModified
                          ? "modified"
                          : ""} ${p.isNewStaged ? "new-staged" : ""}"
                      >
                        <span class="player-name" title="XUID: ${p.xuid}"
                          >${p.name || p.xuid}${p.isNewStaged
                            ? html` <span class="new-tag">(New - Staged)</span>`
                            : ""}</span
                        >
                        <ha-select
                          ...
                          .value=${currentLevel}
                          @selected=${(ev) =>
                            this._handlePermissionLevelChange(
                              p.xuid,
                              ev.target.value,
                            )}
                ...
                <div class="section add-player-section">
                  ...
                  <p class="info">
                    Select a known player OR manually enter XUID. Player name is
                    optional for manual XUID.
                  </p>
                  <ha-select
                    ...
                    helper=${availableGlobalPlayerOptions.length === 0
                      ? "No available known players found"
                      : ""}
                    ?helper-persistent=${availableGlobalPlayerOptions.length ===
                    0}
                  >
                    <mwc-list-item value=""></mwc-list-item>
                    ${availableGlobalPlayerOptions.map(
                      (opt) =>
                        html`<mwc-list-item .value=${opt.value}
                  ...
                  >
                    ${VALID_PERMISSIONS.map(
                      (level) =>
                        html`<mwc-list-item
                          .value=${level}
                          ?selected=${level === this._newPlayerPermissionLevel}
                          >${level}</mwc-list-item
                        >`,
                    )}
                  </ha-select>
        ...
        ${this._selectedServerPermsEntityId && hasStagedChanges
          ? html`
  ...
  static get styles() {
    return css`
      ...
      ha-select {
        --mdc-menu-min-width: calc(100% - 32px); /* Adjust if needed */
      }
      .status-area {
        margin-top: 16px;
        min-height: 1.2em;
      ...
      .permission-entry {
        ...
        align-items: center;
        padding: 8px 4px;
        border-bottom: 1px solid var(--divider-color-light, #eee);
      ...
      .permission-entry.modified {
        background-color: rgba(var(--rgb-primary-color), 0.08);
        border-left: 3px solid var(--primary-color);
        padding-left: 8px;
        margin-left: -11px;
      }
      .permission-entry.new-staged {
        border-left: 3px solid var(--success-color);
      }
      .permission-entry .player-name {
        flex-grow: 1;
        margin-right: 16px;
        font-size: 1em;
        overflow: hidden;
      ...
      .permission-entry .new-tag {
        font-size: 0.8em;
        color: var(--success-color);
        font-weight: bold;
        margin-left: 4px;
      }
      .permission-entry .permission-select {
        width: 130px;
        flex-shrink: 0;
        flex-grow: 0;
        margin-bottom: 0;
  ...
  getCardSize() {
    ...
    if (this.config?.global_players_entity) {
      ...
      if (this._selectedServerPermsEntityId) {
        ...
        const combinedListLength =
          this._currentServerPermissions.length +
          Object.keys(this._editData).filter(
            (xuid) =>
              !this._currentServerPermissions.some((p) => p.xuid === xuid),
          ).length;
...
window.customCards.push({
  ...
  description:
    "View and manage player permissions for a selected Bedrock server.",
  preview: true,
});
_LOGGER.info(
  `%c BSM-PERMISSIONS-CARD %c LOADED (incl. editor) %c`,
  "color: purple; font-weight: bold; background: black",
