...
async def async_handle_send_command_service(service: ServiceCall, hass: HomeAssistant):
    ...
    for entity_id in target_entity_ids:
        ...
        if entity_entry and entity_entry.platform == DOMAIN and entity_entry.config_entry_id:
            ...
            if len(parts) >= 3 and parts[0] == DOMAIN:
                 ...
                 server_name_key = entity_entry.unique_id.replace(f"{DOMAIN}_", "", 1).replace(f"_{parts[-1]}", "", 1)
                 ...
                 if entity_entry.config_entry_id not in servers_to_command:
                     ...
                     _LOGGER.debug("Targeted server '%s' via entity %s", server_name, entity_id)
                      ...
                      _LOGGER.warning("Config entry %s targeted via multiple entities with different inferred server names ('%s' vs '%s'). Using first found.",
                                      entity_entry.config_entry_id, servers_to_command[entity_entry.config_entry_id], server_name)
            else:
                 _LOGGER.warning("Could not determine server name from unique ID '%s' for entity %s", entity_entry.unique_id, entity_id)
        else:
             ...
             if our_entry_id:
                  ...
                  for identifier in device_entry.identifiers:
                      if len(identifier) == 2 and identifier[0] == DOMAIN:
                          ...
                          if ":" not in id_value: # Basic check: server names shouldn't contain colons like host:port
                  ...
                  if server_name_from_dev:
                       ...
                       if our_entry_id not in servers_to_command:
                           ...
                           _LOGGER.debug("Targeted server '%s' via device %s", server_name_from_dev, device_id)
                       elif servers_to_command[our_entry_id] != server_name_from_dev:
                            _LOGGER.warning("Config entry %s targeted via device and entity with different inferred server names ('%s' vs '%s'). Using first found.",
                                            our_entry_id, servers_to_command[our_entry_id], server_name_from_dev)
                  else:
                        _LOGGER.warning("Targeted device %s is the manager device or has unexpected identifiers for domain %s, cannot send server command via this target.", device_id, DOMAIN)
             else:
                  _LOGGER.debug("Targeted device %s not associated with config entry for domain %s", device_id, DOMAIN)
        else:
    ...
    for config_entry_id, target_server_name in servers_to_command.items():
        ...
        if config_entry_id not in hass.data.get(DOMAIN, {}):
            _LOGGER.warning("Targeted config entry %s is not loaded or has no data.", config_entry_id)
            continue
        ...
        try:
            ...
            tasks[config_entry_id] = _async_handle_send_command(
                 target_api, target_server_name, command_to_send # Pass target_server_name
            )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                 ...
                 _LOGGER.error(
                     ...
                     failed_server_name, failed_entry_id, result
                 )
