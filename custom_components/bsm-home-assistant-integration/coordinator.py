    ...
    async def _async_update_data(self) -> dict:
        ...
        try:
            ...
            async with async_timeout.timeout(self._scan_interval - 5):
                 ...
                 status_info = await self.api.async_get_server_status_info(self.server_name)
            ...
            _LOGGER.error("Authentication error fetching data for %s: %s", self.server_name, err)
            raise ConfigEntryAuthFailed(f"Authentication failed for {self.server_name}: {err}") from err
        except ServerNotFoundError as err:
            # Server no longer found by API - potentially deleted on manager side
            _LOGGER.error("Server %s not found by API during update: %s", self.server_name, err)
            ...
            _LOGGER.debug("Server %s is not running during update: %s", self.server_name, err)
            ...
            _LOGGER.warning("Error communicating with API for %s: %s", self.server_name, err)
            raise UpdateFailed(f"Error communicating with API: {err}") from err
        except Exception as err:
             # Catch-all for unexpected errors during update
             _LOGGER.exception("Unexpected error fetching data for %s: %s", self.server_name, err)
